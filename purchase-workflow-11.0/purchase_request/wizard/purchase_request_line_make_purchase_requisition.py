...
class PurchaseRequestLineMakePurchaseRequisition(models.TransientModel):
    ...
    sync_data_planned = fields.Boolean(
        string="Merge on PR lines with equal Scheduled Date")
    ...
    def default_get(self, fields):
        ...
        if len(list(set(groups))) > 1:
            raise UserError(
                _('You cannot create a single purchase requistion from '
                  'purchase request that have different procurement group.'))
        res['item_ids'] = items
    ...
    def _get_purchase_line_onchange_fields(self):
        return ['product_uom_id', 'price_unit']

    @api.model
    def _execute_purchase_line_onchange(self, vals):
        cls = self.env['purchase.requisition.line']
        onchanges_dict = {
    ...
    def _get_order_line_search_domain(self, order, item):
        vals = self._prepare_purchase_requisition_line(order, item)
        name = self._get_purchase_line_name(order, item)
        order_line_data = [('requisition_id', '=', order.id),
                           ('name', '=', name),
...
class PurchaseRequestLineMakePurchaseRequisitionItem(models.TransientModel):
    ...
    keep_description = fields.Boolean(string='Copy descriptions to new PA(QCF)',
