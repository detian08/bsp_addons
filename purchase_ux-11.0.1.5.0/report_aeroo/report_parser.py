...
class ReportAerooAbstract(models.AbstractModel):
    ...
    def complex_report(self, docids, data, report, ctx):
        ...
        model_obj = self.env.get('ir.models')
        model_name = model_obj.search([('models', '=', self.model)])[0].name
        ...
        #_logger.info("End process %s (%s), elapsed time: %s" % (self.name, self.models, time.time() - aeroo_print.start_time), logging.INFO) # debug mode
    ...
    def aeroo_report(self, docids, data):
        ...
        # TODO
        #_logger.info("End Aeroo Reports %s (%s), total elapsed time: %s" % (name, models), time() - aeroo_print.start_total_time), logging.INFO) # debug mode
