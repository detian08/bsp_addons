...
class report_aeroo_import(osv.osv_memory):
    ...
    def install_report(self, cr, uid, ids, context=None):
        ...
        mod_obj = self.pool.get('ir.models.data')
    ...
    def next(self, cr, uid, ids, context=None):
        ...
        if zipfile.is_zipfile(zip_stream):
            ...
            root = tree.getroot()
            info = ''
            report = root.xpath("//data/record[@models='ir.actions.report']")[0]
            style = root.xpath("//data/record[@models='report.stylesheets']")[0]
            rep_name = report.find("field[@name='name']").text
            rep_service = report.find("field[@name='report_name']").text
            rep_model = report.find("field[@name='models']").text
