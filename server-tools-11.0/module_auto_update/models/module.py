...
def ensure_module_state(env, modules, state):
    ...
    if names:
        raise FailedUpgradeError(
            "The following models should be in state '%s' "
...
class Module(models.Model):
    ...
    def upgrade_changed_checksum(self, overwrite_existing_translations=False):
        """Run an upgrade of the database, upgrading only changed models.

        Installed models for which the checksum has changed since the
        ...
        After a successful upgrade, the checksums of installed models are
        saved.
        ...
        _logger.info("Updating models list...")
        self.update_list()
        ...
        if not changed_modules and not self._get_modules_partially_installed():
            _logger.info("No checksum change detected in installed models "
                         "and all models installed, nothing to do.")
            return
        _logger.info("Marking the following models to upgrade, "
        ...
        # would update cheksums for models that have not been upgraded
        ...
        if partial_modules:
            raise IncompleteUpgradeError(
                ...
                "but incomplete for the following models: %s" %
