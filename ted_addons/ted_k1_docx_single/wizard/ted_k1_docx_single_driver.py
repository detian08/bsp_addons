...
class Tedk1DocxWizard(models.Model):
    ...
    def get_data(self):
        ...
        for order in selected_orders:
            if order.doc_type == 'K1':
                ...
                return_value['headers'].append(print_data)
                # page_count += 1
                # return return_value
            # else:
            # return {}
    ...
    def get_result(self):
        ...
        if print_data:
            counter_page = 1
            list_file_result = []
            list_attach_object_id = []
            file_name = ""
            file_template = os.path.join(os.path.dirname(__file__), r'templates\k1_template_v3.docx')
            # file_name = ('test' + str(datetime.today().date()) + 'page' + str(counter_page) + '.docx')
            file_name = ('test' + str(datetime.today().date()) +'.docx')
            # for single_print_data in print_data:
            #     # inisialisasi
            #     file_content = single_print_data
            template_object = DocxTemplate(file_template)
            template_object.render(print_data)
            ...
            warning_msg = {
                'title': 'Warning',
                'message': 'Pilih Dokumen dengan tipe K-1'
            }
